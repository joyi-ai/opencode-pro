...
[**OpenCode**](/) is an open source AI coding agent. It's available as a command-line interface, desktop app, or IDE extension.
...
- **Using Node.js**
      ...
      ```
        ...
        <TabItem label="pnpm">
        ```bash
        pnpm install -g @joyi-ai/openpoo
        ```
        ...
        <TabItem label="Yarn">
        ```bash
        yarn global add @joyi-ai/openpoo
        ```
...
- **Using Mise**
  ...
  ```bash
  mise use -g github:joyi-ai/openpoo
  ```

- **Using Docker**

  ```bash
  docker run -it --rm ghcr.io/joyi-ai/openpoo
  ```

Support for installing OpenCode on Windows using Bun is currently in progress.

You can also grab the binary from the [Releases](https://github.com/joyi-ai/openpoo/releases).
...
1. Run `opencode auth login` or use the desktop app to authenticate at [opencode.ai/auth](https://opencode.ai/auth).
...
Open the desktop app or run `opencode run "<message>"`.
